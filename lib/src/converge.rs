...
pub async fn propose_divergence_solution(
    ...
    if let Some(change_id) = choose_change(converge_ui, &divergent_changes)? {
...
impl TruncatedEvolutionGraph {
    ...
    fn new(
        ...
        // TODO: properly compute the evolution fork point. I think what we want is the
        // "closest choke point" (also known as the "immediate post-dominator") of the
        // set of divergent commits. In graphs with a single initial node, this is
        // guaranteed to exist and is a single node. For now we just use the initial
        // node as the evolution fork point.
        let evolution_fork_point = match initial_nodes.len() {
            1 => Ok(initial_nodes.first().unwrap().clone()),
            _ => {
    ...
    fn get_evolution_fork_point(&self) -> Result<&Commit, ConvergeError> {
        let commit_id = self.evolution_fork_point.clone();
        let fork_point = self.nodes.get(&commit_id).unwrap();
...
{
    let divergent_values: HashSet<T> = divergent_commits.iter().map(attr_fn).collect();
    if divergent_values.len() == 1 {
        return Ok(divergent_values.into_iter().next().unwrap());
    }
    let merge = create_merge(graph, attr_fn)?;
...
fn solve_author(
    ...
    let attr_fn = |c: &Commit| c.author().clone();
    ...
    solve(divergent_commits, graph, attr_fn, ui_chooser)
}
...
fn solve_description(
    ...
    let attr_fn = |c: &Commit| c.description().to_string();
    let ui_chooser = |commits: &[Commit], graph: &TruncatedEvolutionGraph| {
...
fn solve_parents(
    ...
    if viable_parents.len() == 1 {
        return Ok(viable_parents.into_iter().next().unwrap());
    }
...
fn remove_descendants(
    ...
    let revset_expression = Arc::new(RevsetExpression::Commits(commit_ids.to_vec())).minus(
        &Arc::new(RevsetExpression::Commits(commit_ids.to_vec()))
            .children()
            .descendants(),
    );
...
async fn converge_trees(
    ...
    let parent_commits: Vec<Commit> = parents
        ...
        .try_collect()?;
    let parents_merged_tree = merge_commit_trees(repo.base_repo(), &parent_commits).await?;

    let fork_point_tree = fork_point.tree();
    let fork_point_parent_tree = fork_point.parent_tree(repo.base_repo())?;
    ...
    for divergent_commit_id in divergent_commit_ids {
        let divergent_commit = repo.store().get_commit(divergent_commit_id)?;
        terms.push((
            divergent_commit.tree(),
            format!("divergent commit {divergent_commit_id:.12}"),
        ));
        terms.push((
            divergent_commit.parent_tree(repo.base_repo())?,
...
fn create_merge<T>(
    ...
    merge_builder.extend([attr_fn(fork_point)]);
    for node in graph.nodes.values() {
        for predecessor in &node.predecessors {
            let predecessor_node = graph.nodes.get(predecessor).unwrap();
